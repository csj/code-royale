import { ErrorLog } from '../core/ErrorLog.js';
import { WIDTH, HEIGHT } from '../core/constants.js';
import * as utils from '../core/utils.js';
import { api as entityModule } from '../entity-module/GraphicEntityModule.js';


function getMouseOverFunc(id, tooltip) {
  return function () {
    tooltip.inside[id] = true;
  };
}

function getMouseOutFunc(id, tooltip) {
  return function () {
    delete tooltip.inside[id];
  };
}

function getEntityState(entity, frame, progress) {
  const subStates = entity.states[frame];
  if (subStates && subStates.length) {
    return subStates[subStates.length - 1];
  }
  return null;
}

function getMouseMoveFunc(tooltip, container, module) {
  return function (ev) {
    if (tooltip) {
      var pos = ev.data.getLocalPosition(container);
      tooltip.x = pos.x;
      tooltip.y = pos.y;
      var point = {
        x: pos.x * entityModule.coeff,
        y: pos.y * entityModule.coeff
      };
      var showing = null;
      var ids = Object.keys(tooltip.inside).map(n => +n);
      var entity;

      for (let id of ids) {
        if (tooltip.inside[id]) {
          entity = entityModule.entities.get(id);
          if (!entity) {
            delete tooltip.inside[id];
          } else {
            showing = id;
          }
        }
      }

      let state = null;
      if (showing !== null) {
        state = getEntityState(entity, module.currentFrame);
      }

      if (state !== null) {
        const params = module.registered[showing];

        tooltip.label.text =
            (params.hasOwnProperty('id') ? 'id: ' + params.id + '\n' : '')
          + 'type: ' + params.type
          + '\nx: ' + state.x
          + '\ny: ' + (state.y - hudHeight)
          ;

        tooltip.visible = true;
        const extras = module.extra[showing];

        if (extras) {
          const frames = Object.keys(extras).map(a => +a).sort((b,c) => b - c);
          let index = 0;
          while (index < frames.length - 1 && frames[index] < module.currentFrame) {
            index++;
          }
          const extra = extras[frames[index]];
          if (extra) {
            tooltip.label.text += "\n" + extra.join("\n")
          }
        }
      } else {
        tooltip.visible = point.y <= 1080 && point.y >= hudHeight && point.x >= 0 && point.x < WIDTH;
        tooltip.label.text = 'x: ' + Math.round(point.x) + '\ny: ' + Math.round(point.y - hudHeight);
      }

      tooltip.background.width = tooltip.label.width + 20;
      tooltip.background.height = tooltip.label.height + 20;
      // scope.drawer.asyncRenderingTime = Drawer.RenderTimeout;
      tooltip.pivot.x = -30;
      tooltip.pivot.y = -50;

      if (tooltip.y - tooltip.pivot.y + tooltip.height > HEIGHT) {
        tooltip.pivot.y = 10 + tooltip.height;
      }

      if (tooltip.x - tooltip.pivot.x + tooltip.width > WIDTH) {
        tooltip.pivot.x = tooltip.width;
      }

    }
  };
}

var hudHeight = 0;

export class TooltipModule {
  constructor(assets) {
    this.registered = {};
    this.extra = {};
    this.lastProgress = 1;
    this.lastFrame = 0;
    this.toDestroy = [];
  }

  static get name() {
    return 'tooltips';
  }

  updateScene(previousData, currentData, progress) {
    this.currentFrame = currentData.number;
    this.currentProgress = progress;
  }

  handleFrameData(frameInfo, [registrations, extra]) {

    // Quick check
    Object.keys(registrations).forEach(
      k => {
        if (this.registered[k]) {
          ErrorLog.push(new NotYetImplemented('Registering the same entity multiple times', state.image));
        }
      }
    );

    Object.assign(this.registered, registrations);

    Object.keys(extra).forEach(
      k => {
        if (!this.extra[k]) {
          this.extra[k] = {};
        }
        this.extra[k][frameInfo.number] = extra[k];
      }
    );
    return { number: frameInfo.number };
  }

  reinitScene(container, canvasData) {
    this.toDestroy.forEach(texture => {
      try {
        texture.destroy(true);
      } catch (ouch) {
        // ignore, carry on
      }
    });

    this.tooltip = this.initTooltip();
    entityModule.entities.forEach(entity => {
      if (this.registered[entity.id]) {
        entity.container.interactive = true;
        entity.container.mouseover = getMouseOverFunc(entity.id, this.tooltip);
        entity.container.mouseout = getMouseOutFunc(entity.id, this.tooltip);
      }
    });
    this.container = container;
    container.interactive = true;
    container.mousemove = getMouseMoveFunc(this.tooltip, container, this);
    container.addChild(this.tooltip);
  }

  generateText(text, size, color, align) {
    var textEl = new PIXI.Text(text, {
      fontSize: Math.round(size / 1.2) + 'px',
      fontFamily: 'Lato',
      fontWeight: 'bold',
      fill: color
    });

    textEl.lineHeight = Math.round(size / 1.2);
    if (align === 'right') {
      textEl.anchor.x = 1;
    } else if (align === 'center') {
      textEl.anchor.x = 0.5;
    }
    this.toDestroy.push(textEl);
    return textEl;
  };

  initTooltip() {
    var tooltip = new PIXI.Container();
    var background = tooltip.background = new PIXI.Graphics();
    var label = tooltip.label = this.generateText('', 36, 0xFFFFFF, 'left');

    background.beginFill(0x0, 0.7);
    background.drawRect(0, 0, 200, 185);
    background.endFill();
    background.x = -10;
    background.y = -10;

    tooltip.visible = false;
    tooltip.inside = {};

    tooltip.addChild(background);
    tooltip.addChild(label);

    tooltip.interactiveChildren = false;
    return tooltip;
  };

  animateScene(delta) {

  }

  handleGlobalData(players, globalData) {

  }
}

class NotYetImplemented extends Error {
  constructor(feature) {
    super('Not yet implemented: "' + feature);
    this.feature = feature;
    this.name = 'NotYetImplemented';
  }
}